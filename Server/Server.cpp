...
Server::Server(){
    ...
    acceptor.get()->setChannel(mainepoller.get());
    ...
    for(int i=0;i<threadnum;i++){
        subReactors.emplace_back(std::make_shared<Epoller>());
    }
    for(int i=0;i<threadnum;i++){
...
void Server::connectionEstablished(Socket* _socket){
    ...
    connections[fd]=std::make_unique<Connection>(new Channel(fd));
    connections[fd].get()->setChannel(subReactors[random].get());
}